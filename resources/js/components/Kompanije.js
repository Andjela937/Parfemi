...
export default class Kompanije extends Component {
    ...
    ucitavanje() {
        axios.get("http://127.0.0.1:8000/kompanije/get").then(res => {
            ...
            kompanije.map(async kompanija => {
                kompanija.parfemi = await this.ucitavanjeParfema(
                    kompanija.id
                );
    ...
    async ucitavanjeParfema(kompanija_id) {
        const response = await axios.get(
            "http://127.0.0.1:8000/kompanije/get-parfemi?kompanija_id=" +
                kompanija_id
        );
        return response.data.parfemi;
    }
